        ...
        return (
            <View style={{ height: height * 0.25, justifyContent: 'space-between', paddingHorizontal: 20, backgroundColor: 'white' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 20 }}>
                    {slides.map((_, index) => (
                        <View key={index} style={[styles.indicator, currentIndex === index && { backgroundColor: '#87b5eb', width: 10,height:10,borderRadius:50 }]} />
                    ))}
                </View>
                <View style={{ marginBottom: 0 }}>
                    {currentIndex === slides.length - 1 ? (
                        <View style={{ height: 50 }}>
                            <TouchableOpacity
                                style={[styles.btn, { backgroundColor: 'skyblue' }, { borderColor: 'white', borderWidth: 1,borderRadius:40, }]}
                                onPress={() => navigate("Home")}
                            >
                                <Text style={{ fontWeight: 'bold', fontSize: 15,color:"#ffff" }}>Get Started</Text>
                            </TouchableOpacity>
                        </View>
                    ) : (
