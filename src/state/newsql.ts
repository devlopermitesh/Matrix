...
import SQLite, { SQLError, SQLTransaction, WebsqlDatabase } from 'react-native-sqlite-2';
import SQLiteDatabase from 'react-native-sqlite-2/lib/typescript/SQLiteDatabase';
...
private init=async():Promise<boolean>=>{
 ...
 this.db=db;
   if(!db){
    ...
    const response=await this.createTabletodos(db)
   return true
}
  ...
  return await new Promise<void>((resolve, reject) => {
    this.db!.transaction((tx) => {
      tx.executeSql(
        ...
        [item.name, item.description, dueDateString, Categories[item.category], item.iscompleted ? 1 : 0],
        (data) => {
...
// Update a todo item
public updateTodo = async (id: string, item: Partial<Item>): Promise<Item|null> => {
...
public getTodosByCompletion = async (isCompleted: boolean): Promise<Item[]> => {
 ...
 private getDBConnection = async (): Promise< WebsqlDatabase> => {
     return SQLite.openDatabase(this.database_name,this.database_version,this.database_displayname,this.database_size,()=>{
        
     });
  }
ErrorDisplay=(err: SQLError)=>{
