...
export const useTodos = create<TodosState>((set, get) => ({
  todos: Object.keys(Categories).filter((key) => isNaN(Number(key))).reduce((acc, key) => {
...
const groupedTodos = Object.keys(Categories)
 ...
 newtodo:async(data:Item)=>{
    ...
    await get().setData();
    await trigger.setnotify(data.name,data.description.slice(0,17),new Date(data.dueDate),data.id)
...
await get().setData();
if(data){
await trigger.updatenotify(id,data.name,data.description.slice(0,16),new Date(data.dueDate))
}
 },
