  ...
  return ({ children, title }: { children: React.ReactNode; title: string }) => {
    ...
    return (
      <View testID={'button-container-settings'}>
        <Text>{title}</Text>
        <View testID={`button-container-${title.toLowerCase().replace(/\s+/g, '-')}`}>
          {children}
        </View>
      </View>
    );
...
jest.mock('../../../../src/components/atoms/Icon', () => {
  return ({ name, size, color, iconType }: { name: string; size: number; color: string; iconType: string }) => {
    const { Text } = require('react-native');
    return <Text testID={`icon-${name}-checkout`}>{`Icon-${name}-${iconType}`}</Text>;
  };
});
