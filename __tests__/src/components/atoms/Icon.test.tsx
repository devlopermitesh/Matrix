...
describe("Icon Component", () => {
  ...
  it("renders MaterialCommunity icon", () => {
    const { getByLabelText } = render(<Icon name="account" iconType="material-community" />);
    expect(getByLabelText("MaterialCommunity")).toBeTruthy();
  });

  it("renders FontAwesome icon", () => {
    const { getByLabelText } = render(<Icon name="star" iconType="font-awesome" />);
    expect(getByLabelText("FontAwesomeIcon")).toBeTruthy();
  });

  it("renders Ionicons icon", () => {
    const { getByLabelText } = render(<Icon name="ios-home" iconType="ionicons" />);
    expect(getByLabelText("IoniconsIcon")).toBeTruthy();
  });

  it("renders AntDesign icon", () => {
    const { getByLabelText } = render(<Icon name="plus" iconType="ant-design" />);
    expect(getByLabelText("AntDesignIcon")).toBeTruthy();
  });

  it("renders Octicons icon", () => {
    const { getByLabelText } = render(<Icon name="alert" iconType="Octicons" />);
