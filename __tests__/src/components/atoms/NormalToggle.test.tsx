...
describe("SmoothToggle Component", () => {
  ...
  it("calls onPress when pressed", () => {
    ...
    const { getByRole } = render(<SmoothToggle isOn={false} onPress={mockPress} />);
  ...
  it("renders with isOn=false (off state)", () => {
    const { getByTestId } = render(<SmoothToggle isOn={false} onPress={() => {}} />);
    expect(getByTestId("track").props.style.backgroundColor).toBe("#e5e7eb");
  });

  it("renders with isOn=true (on state)", () => {
    const { getByTestId } = render(<SmoothToggle isOn={true} onPress={() => {}} />);
    expect(getByTestId("track").props.style.backgroundColor).toBe("#e5e7eb");
  });

  it("applies small size preset correctly", () => {
    const { getByTestId } = render(<SmoothToggle isOn={false} onPress={() => {}} size="small" />);
  ...
  it("applies large size preset correctly", () => {
    const { getByTestId } = render(<SmoothToggle isOn={false} onPress={() => {}} size="large" />);
    const track = getByTestId("track");
